...
// see keys.h, gitignore don't include keys for security :clown:
#include "keys.h"
//------------------------------------------------------------------
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (uart_cmd == "SENSOR1") {
      Serial.print("PH:");
      Serial.println(PH.get_last_received_reading(), 2);
    } else if (uart_cmd == "SENSOR2") {
      Serial.print("EC:");
      Serial.println(EC.get_last_received_reading(), 0);
